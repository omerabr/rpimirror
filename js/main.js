...
function dragToTop(){
	$('.center-ver.center-hor').animate({ "top": "-=37%" ,"fontSize": "-=25px","opacity": "1"}, 500,function(){
		isAtBottom = false;
		$('.center-ver.center-hor').attr("data-place","up");
	} ).delay(1000);
	
	
}
function dragToBottom(){
	
	$('.center-ver.center-hor').animate({ "top": "+=37%" ,"opacity": "0"}, "slow",function(){
		$('.center-ver.center-hor').css({ 'font-size': "+=25px" });
		isAtBottom = true;
		$('.center-ver.center-hor').attr("data-place","down");
	} );
	//$('.center-ver.center-hor').fadeOut(1000);
	//$('.user').updateWithText('',1);
	
	
	//console.log("dragToBottom() "+ isAtBottom);
}
...
jQuery(document).ready(function($) {
	...
	//{	
		//if($('.warning.noWebcam').css('display') != 'block'){
			//	$('.trigger').click();
		//}
		//setTimeout(function() {
			//checkCamera();
	...
	(function updateCompliment()
	{
      ...
      if (hour >= 17 || hour < 3) compliments = evening;
		...
		//if (person && person.toLowerCase().indexOf("omer") >= 0){ $('.compliment').updateWithText('Hi Omer!')};
	...
	function updateProfile(result){
			...
			if (result.photos[0].tags[0]){
				debugger;
				person = result.photos[0].tags[0].uids[0].uid;
				alert(person);
				//console.log(result.photos[0].tags[0].uids[0].prediction);
				if (person != lastPerson || $('.center-ver.center-hor').attr("data-place")=="down"){
					if (person && person.toLowerCase().indexOf("omer") >= 0){
						$('.user').updateWithText('Hi Omer!',1);
						lastPerson=person;
						person = null;
						
						setTimeout(function() {
							if ($('.center-ver.center-hor').attr("data-place")=="down"){
					...
					else if (person && person.toLowerCase().indexOf("tzahit") >= 0){
						$('.user').updateWithText('Hi Tzahit!',1);		
						lastPerson=person;
						person= null;
						setTimeout(function() {
							if ($('.center-ver.center-hor').attr("data-place")=="down"){
					...
					else{if($('.center-ver.center-hor').attr("data-place")=="up"){dragToBottom();}person = null;};
				...
				console.log("nada");
				person= null;
				if($('.center-ver.center-hor').attr("data-place")=="up"){
					dragToBottom();
				};
	...
	function uploadImage(file) {
		...
		fd.append("attributes", 'all');
		fd.append("detector", 'fast');
		fd.append("limit", '64');
		fd.append("namespace", 'magic.mirror');
		fd.append("uids", 'all');
		// And send it
		$.ajax({
			url: "https://face.p.mashape.com/faces/recognize?api_key=293fa06116c14fc59b4436a99b397e8b&api_secret=40b245719a1f4dfc8887aabb1ff147ff",
			type: "POST",
			data: fd,
			processData: false,
			contentType: false,
			beforeSend: function (xhr) {
				xhr.setRequestHeader("X-Mashape-Key", 'FHZZp0r1O7mshpHei96uCMrG5M2mp12tVjijsntt2xZtMN8DvW');
			}
